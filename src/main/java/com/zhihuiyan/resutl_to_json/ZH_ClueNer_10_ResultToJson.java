...
public class ZH_ClueNer_10_ResultToJson {
	...
	static {		
		...
		mybertTonkenizer = new MyBertTokenizers(vocabel_file_name);
		try {
			readJsonFiles();
		} catch (IOException e) {
	...
	public static void readJsonFiles() throws IOException {
		...
		String json_file_1 = new String("/text_parsing/src/main/resources/model_resource/json_of_labels_10_clue_ner.json");
	...
	public static List<List<List<Entity>>> predictPosProcess(String [] stringTextList,int[][] result_list) throws IOException {
		...
		TokensData result_tokens_of_strings = mybertTonkenizer.getInputs(stringTextList);
	...
	private static List<List<Entity>> result_to_json(List<String> tokens, List<String> tags) {
		...
		String[] chinese_name = {"地址","书名","公司","游戏","政府","电影名","姓名","组织机构","职位","风景地名"};
		String[] english_name = {"address","book","company","game","government","movie","name","organization","position","scene"};
